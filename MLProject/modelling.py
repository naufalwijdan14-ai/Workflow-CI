...
# REVISI: Aktifkan autolog saja (Mencakup >2 artefak secara otomatis)
mlflow.sklearn.autolog(log_models=True)

def train_model():
    data_path = "titanic_preprocessing.csv"
    if not os.path.exists(data_path):
        data_path = "MLProject/titanic_preprocessing.csv"
    ...
    with mlflow.start_run(run_name="Advance_Run_GitHub_Actions"):
        model = RandomForestClassifier(n_estimators=100, max_depth=5, random_state=42)
        model.fit(X_train, y_train)
        
        joblib.dump(model, "model.pkl")
        
        print("Training selesai. Autolog telah mencatat metrik dan artefak.")
